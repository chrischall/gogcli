...
// Regression coverage for the file-input body flags reporting a
// trailing-newline-trimmed length in --dry-run output. The bytes handed to
// --body-file/--body-html-file are sent verbatim (quoted-printable preserves
// them), so the reported body_len/body_html_len must match the file size and
// stay consistent with the inline flags and with slides update-notes.
//
