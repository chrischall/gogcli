...
- Gmail: stop `drafts update` anchoring `In-Reply-To`/`References` to the draft's own previous revision, keeping thread continuity separate from reply lineage, validating stored reply headers against sent or received messages, automatically repairing stale legacy context from the nearest surviving ancestor or latest real thread message, preserving genuine context across repeated updates without growing `References`, refusing drafts as reply targets, reporting whether context was explicit, preserved, or repaired, and adding `--clear-reply-context`. (#942, #944)
